...
import { _spawnPromise, validateUrl, isYouTubeUrl, sanitizeFilename } from "./utils.js";
/**
 ...
 * ```
 */
export async function downloadVideo(url, config, resolution = "720p", startTime, endTime, chapter) {
    ...
    try {
        ...
        if (startTime || endTime) {
            let sectionArgs = [];
            if (startTime)
                sectionArgs.push("-ss", startTime);
            if (endTime)
                sectionArgs.push("-to", endTime);
