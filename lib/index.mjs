...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    if (toolName === "list_subtitle_languages") {
        ...
        return handleToolExecution(() => downloadVideo(args.url, CONFIG, args.resolution, args.startTime, args.endTime, args.chapter), "Error downloading video");
    }
        ...
        return handleToolExecution(() => downloadTranscript(args.url, args.language || CONFIG.download.defaultSubtitleLanguage, CONFIG), "Error downloading transcript");
    }
    else if (toolName === "list_chapters") {
        return handleToolExecution(() => listChapters(args.url), "Error listing chapters");
    }
    else {
        return {
